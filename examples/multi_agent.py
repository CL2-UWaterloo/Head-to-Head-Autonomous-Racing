...
def main():
    ...
    while not done:
        ego_x, ego_y, ego_theta = obs['poses_x'][0], obs['poses_y'][0], obs['poses_theta'][0]
        opp_x, opp_y, opp_theta = obs['poses_x'][1], obs['poses_y'][1], obs['poses_theta'][1]
        
        dist = np.sqrt((ego_x-opp_x)**2 + (ego_y-opp_y)**2)
        print(f"Distance between vehicles: {dist}")
        
        ego_progress = planner.get_progress_along_track(np.array([ego_x, ego_y]))
        opp_progress = planner.get_progress_along_track(np.array([opp_x, opp_y]))
        
        # This logic breaks after completing a lap. Ex: ego is at 0.9, opp is at 0.1, but opp is actually ahead
        # But the basic idea is to increase the velocity gain of the car that is behind
        ego_vgain = work['vgain']
        opp_vgain = work['vgain']
        ...
        ego_speed, ego_steer = planner.plan(ego_x, ego_y, ego_theta, work['tlad'], ego_vgain)
        opp_speed, opp_steer = planner.plan(opp_x, opp_y, opp_theta, work['tlad'], opp_vgain)
